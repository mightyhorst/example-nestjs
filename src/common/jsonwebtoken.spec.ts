...
it.skip('should generate a JWT token under 8kb using a user with a large number of group memberships', async () => {
    ...
    // For URL Length (MAX URL Limit)
    expect(token.length).toBeLessThanOrEqual(2000);

    // For Authorization: header (MAX Header Limit)
