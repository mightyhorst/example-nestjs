...
**/
import { 
	WarrantyResponseDto as ResDto, 
	// WarrantyErrorResponseDto as ErrDto, 
	WarrantyChartResponseDto  as ChartDto
} from '../dto/index';
import { 
...
export class WarrantyController {
	...
	/**
	* @name get es query 
	* @description GET /api/v2/warranty/{sitecode}
	* @description Directly query Elastic Search
	* @param 	{params.sitecode} 	sitecode - school ID
	* @param 	{query.formfactor} 	formfactor - filter based on form factor
	...
	**/
	...
	async search(
		...
		try{
			let searchResults = await this.warrantyService.search(
				sitecode, 
				formFactor,
				warranty, 
				ageOfDevice,
	...
	/**
	* @name get chart data 
	* @description GET /api/v2/warranty/{sitecode}/chart
	...
	* @param 	{query.age} 		ageOfDevice - filter based on age of device
	* @param 	{query.utilisation} utilisation - filter based on utilisation
	* @param 	{query.model} 		model - filter based on modelCount
	* @param 	{query.size} 		size - limit the size of the results
	* @returns 	{ChartDto} 
	* @returns 	{AuthErrorRes} Unauthorized error 
	* @returns 	{ValidErrorRes} Validation errors 
	...
	**/
	...
	async chart(
		...
		try{
			let searchResults = await this.warrantyService.searchToChart(
				sitecode, 
				formFactor,
				warranty,
				ageOfDevice,
